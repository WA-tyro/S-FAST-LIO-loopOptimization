...
namespace esekfom
{
	...
	public:
		...
		state_ikfom boxplus(state_ikfom x, Eigen::Matrix<double, 24, 1> f_)
		{
			...
			x_r.rot = x.rot * MySO3::exp(f_.block<3, 1>(3, 0) );
			x_r.offset_R_L_I = x.offset_R_L_I * MySO3::exp(f_.block<3, 1>(6, 0) );
		...
		vectorized_state boxminus(state_ikfom x1, state_ikfom x2 )
		{
			...
			x_r.block<3, 1>(3, 0) = MySO3( x2.rot.matrix().transpose() * x1.rot.matrix() ).log() ;
			x_r.block<3, 1>(6, 0) = MySO3( x2.offset_R_L_I.matrix().transpose() * x1.offset_R_L_I.matrix() ).log() ;
